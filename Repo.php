...
class Repo {
	...
	public function updateImages( $image_id,  $new_alt_text ) {
		...
		if ( ! isset( $_POST[ $image_id ] ) ) {
			$this->flashMessage( 'error-no-img-selected', __('Please select at least one image',TXD) );
			return false;
		}

		if ( ! isset( $_POST[$new_alt_text] ) )
		{
			$this->flashMessage( 'error-no-alt', __('please select tag or write something!',TXD) );
			return false;
	...
	public function updateMultipleItemsBySingleAlt( $ids, $new_alt ) {
		if ( ! isset( $_POST[ $ids ] ) )
		{
			$this->flashMessage( 'error-no-img-selected', __('you forgot to select any images',TXD) );
			return false;
		}
	...
	public function showGalleries() {
		...
		echo '<select class="gallery-select" onchange="location = this.options[this.selectedIndex].value;"><option value="?page='.$_REQUEST['page'].'">'.__('all galleris',TXD).'</option>';
	...
	public function inputCheckboxToArray( $checkbox, $text_input ) {
		...
		if ( ! isset( $new_alt ) and ! isset( $custom_alt_to_array ) ) {
			...
			$this->flashMessage( 'no-alt', __('please select tag or write something!',TXD) );
			return false;
